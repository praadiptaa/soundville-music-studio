...
class BookingModel {
  /**
   * Memeriksa apakah jadwal studio tersedia pada waktu yang diinginkan (Global studio check)
   */
  static async isScheduleAvailable(id_studio, tanggal, jam_mulai, jam_selesai, excludeId = null) {
    ...
    console.log(`[DEBUG] isScheduleAvailable (Global Check) - Tanggal: ${tanggal}, Jam: ${jam_mulai}-${jam_selesai}, Available: ${rows[0].total === '0' || rows[0].total === 0}`);
    return rows[0].total === '0' || rows[0].total === 0;
  }

  /**
   * Mengambil jadwal booking pada tanggal tertentu untuk seluruh studio (Shared schedule)
   */
  static async getScheduleByStudioAndDate(id_studio, tanggal) {
    console.log(`[DEBUG] getScheduleByDate (Global Check) - Tanggal="${tanggal}"`);
    const [rows] = await db.query(
      `SELECT b.id_booking, b.jam_mulai, b.jam_selesai, b.status_booking, b.tanggal, b.id_studio,
  ...
  /**
   * Jadwal booking dalam rentang bulan (Shared schedule)
   */
  static async getScheduleByStudioAndMonth(id_studio, year, month) {
    const [rows] = await db.query(
      `SELECT TO_CHAR(b.tanggal, 'YYYY-MM-DD') as tanggal, b.jam_mulai, b.jam_selesai, b.status_booking, b.id_studio
       FROM bookings b
       LEFT JOIN payments p ON b.id_booking = p.id_booking
       WHERE EXTRACT(YEAR FROM b.tanggal) = ?
    ...
    console.log(`[DEBUG] getScheduleByMonth (Global Check) - Year: ${year}, Month: ${month}, Hasil: ${rows.length} booking`);
    return rows;
  }
